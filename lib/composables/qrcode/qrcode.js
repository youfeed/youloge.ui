...
export default (text = "youloge.com", options = {}) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const qrcode = new QRCode(opts.typeNumber, opts.correctLevel);
      qrcode.addData(opts.text);
      qrcode.make();

      const canvas = document.createElement("canvas");
      canvas.title = opts.text;
      canvas.width = opts.width;
      canvas.height = opts.height;
      const ctx = canvas.getContext("2d");

      const tileW = opts.width / qrcode.getModuleCount();
      const tileH = opts.height / qrcode.getModuleCount();

      for (let row = 0; row < qrcode.getModuleCount(); row++) {
        for (let col = 0; col < qrcode.getModuleCount(); col++) {
          ctx.fillStyle = qrcode.isDark(row, col) ? opts.foreground : opts.background;
