let creat = document.createElement('style');creat.title = 'dynamic';document.head.appendChild(creat);
let sheet = creat.sheet;
let span = Array.from(new Array(24),(val,index)=>index+1);
...
let style = [
  ...
  `@font-face {font-family: "youloge";src: url('//at.alicdn.com/t/c/font_3903654_gqwxlte58mr.ttf?t=1676756880103') format('truetype');}`,
  `.y-icon{width: 1em; height: 1em;vertical-align: -0.15em;fill: currentColor;overflow: hidden;}.y-icon svg { height: 1em; width: 1em; }`,
  ...
  // `.y-mask{position: fixed; top: 0; right: 0; bottom: 0; left: 0; display: flex; align-items: center; justify-content: center; background: #00000047; z-index: 999999;}`,
  `.v-cloak{display: none}`
];
// 水波纹
style.push(`.y-ripple{position: absolute;transform: translate(-50%,-50%);  pointer-events: none; overflow: hidden;background: currentColor; border-radius: 50%;}`)
...
Object.entries({primary:'409EFF',success:'67C23A',info:'E6A23C',warning:'F56C6C',danger:'909399',text:'409eff'}).forEach(([type,hex])=>{
  sheet.insertRule(`.y-message-${type}{border-left: 4px solid #${hex};}`)
  sheet.insertRule(`
    .y-button-${type}{
      cursor: pointer;
      user-select: none;
