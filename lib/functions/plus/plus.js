...
const onCreate = (config={})=>{
  let hash = `#${Math.random().toString(32).substring(2)}`;
  // 动态加载
  ...
  window.addEventListener('message',(({origin,data,source})=>{
    ...
    if(method && URL.startsWith(origin) && keys === hash){
      let work = {
        ...
        'success':()=>{App.unmount();resolve && resolve.call(this,params)},
        'error':()=>{App.unmount();reject && reject.call(this,params)}
      };
  ...
  promise.listener = event => EventBus[hash].event = event;
  return promise;
}

export default {name:'usePlus',use:function(cfg={}){
  let {ukey} = JSON.parse(sessionStorage.getItem('youloge'));
  const pay = (deploy)=>onCreate({...deploy,...{method:'youloge.pay',ukey:ukey},...cfg})
  const sso = (deploy)=>onCreate({...deploy,...{method:'youloge.sso',ukey:ukey},...cfg})
  return {pay,sso}
}};