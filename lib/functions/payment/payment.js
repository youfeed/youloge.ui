...
const usePayment = (cfg={})=>{
  let {ukey} = JSON.parse(sessionStorage.getItem('youloge')),config = Object.assign({ ukey:ukey,close:true,order:{} },cfg),App =null;
  ...
  const pay = (deploy)=>{
    const params = Object.assign({method:'pay'},deploy,config);
