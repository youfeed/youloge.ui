...
const components = {HashView,yAd,yTable,yLink,yButton,yHeader,yRich,yDiscuss,yRgb,yVideo,yLogin,yPayment,ySearch};
export {HashView,yAd,yTable,yLink,yButton,yHeader,yRich,yDiscuss,yRgb,yVideo,yLogin,yPayment,ySearch}
// 不要自动安装插件：要配置参数
//
export const install = (app,config)=>{
  console.log('youloge install',app)
  let {APIKEY,APIURL,VIPURL,ROUTES} = config;
  let session = Object.assign({APIKEY:'',APIURL:'',VIPURL:'',WWWURL:'',OPENURL:'https://open.youloge.com',ROUTES:[]}, {APIKEY,APIURL,VIPURL,ROUTES});
