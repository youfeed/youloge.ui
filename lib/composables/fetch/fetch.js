...
export default function useFetch(cfg = {}) {
    let { APIURL, VIPURL, APIKEY } = useConfig(), config = { ...{ mask: true, duration: 8000 }, ...cfg };
    const request = (url = '', options = {}, type = 'json') => {
        let loading = config.mask ? useLoading() : null;
    ...
    const api = (path = '', method = '', params = {}) => request(`${APIURL}/${path}`, { method: 'post', headers: { "Content-Type": "application/json", apikey: APIKEY }, body: JSON.stringify({ method: method, params: params }) })
